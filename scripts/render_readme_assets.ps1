...
$stdout = Join-Path $env:TEMP ("dbt-multi-touch-attr-" + [guid]::NewGuid().ToString() + "-stdout.log")
$stderr = Join-Path $env:TEMP ("dbt-multi-touch-attr-" + [guid]::NewGuid().ToString() + "-stderr.log")
$port = "5274"
$process = $null
$edgeCandidates = @(
...
try {
    ...
    $targets = @(
        ...
        @{ Url = "http://127.0.0.1:$port/model-lane"; File = "02-model-lane-proof.png"; Size = "1600,1180" },
        @{ Url = "http://127.0.0.1:$port/journey-credit"; File = "03-journey-credit-proof.png"; Size = "1600,1500" },
